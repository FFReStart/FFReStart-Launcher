...
public sealed class GameAccountRepository
{
    ...
    public AuthenticationResult Authenticate(string username, char[] password)
    {
        ...
        try
        {
            ...
            return AuthenticationResult.Succeeded(normalizedUsername, expectedHash);
        }
        finally
        {
    ...
    public AuthenticationResult ValidateRememberedSession(string username, ReadOnlySpan<byte> accountSecret)
    {
        try
        {
            ...
            try
            {
                return CryptographicOperations.FixedTimeEquals(currentSecret, accountSecret)
                    ? AuthenticationResult.Succeeded(normalizedUsername, currentSecret)
