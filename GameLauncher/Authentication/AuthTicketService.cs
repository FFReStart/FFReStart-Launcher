...
public sealed class AuthTicketService
{
    ...
    private static readonly byte[] SigningKeyContext = Encoding.ASCII.GetBytes("FFReStart.AuthTicket.SigningKey.v1");
    private static readonly byte[] SubjectContext = Encoding.ASCII.GetBytes("FFReStart.AuthTicket.Subject.v1");
    ...
    public string Create(ReadOnlySpan<byte> accountSecret)
    {
        ...
        {
            throw new ArgumentException("Ticket key material must be 32 bytes.", nameof(accountSecret));
        }
        ...
        byte[] subjectBytes = HMACSHA256.HashData(accountSecret, SubjectContext);
        byte[]? signature = null;
        ...
        try
        {
            ...
            signature = HMACSHA256.HashData(accountSecret, payloadBytes);
    ...
    public static byte[] DeriveSigningKey(ReadOnlySpan<byte> accountVerifier) =>
        HMACSHA256.HashData(accountVerifier, SigningKeyContext);
    ...
    public bool Validate(string ticket, ReadOnlySpan<byte> accountSecret)
    {
        ...
        try
        {
            byte[] expectedSubjectBytes = HMACSHA256.HashData(accountSecret, SubjectContext);
