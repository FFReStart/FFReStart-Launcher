...
public sealed class AuthenticationTests : IDisposable
{
    ...
    [Fact]
    public void TicketKeyIsDomainSeparatedFromPasswordVerifier()
    {
        byte[] verifier = Enumerable.Range(1, 32).Select(value => (byte)value).ToArray();
        byte[] expected = HMACSHA256.HashData(
            verifier,
            Encoding.ASCII.GetBytes("FFReStart.AuthTicket.SigningKey.v1"));
        byte[] actual = AuthTicketService.DeriveSigningKey(verifier);
        try
        {
            Assert.Equal(expected, actual);
            Assert.False(CryptographicOperations.FixedTimeEquals(verifier, actual));
        }
        finally
        {
    ...
    [Fact]
    public void GameLaunchUsesStructuredTokenArgumentsWithoutShell()
    {
        const string fixtureTicket = "v1.fixture.subject.expiry.nonce.signature";
        ProcessStartInfo startInfo = GameLaunchCommand.Create(@"C:\Games\FFReStart\Game.exe", fixtureTicket);

        Assert.False(startInfo.UseShellExecute);
        Assert.Equal(@"C:\Games\FFReStart\Game.exe", startInfo.FileName);
        Assert.Equal(new[] { "--auth-token", fixtureTicket }, startInfo.ArgumentList);
        Assert.DoesNotContain(fixtureTicket, startInfo.FileName, StringComparison.Ordinal);
    }
